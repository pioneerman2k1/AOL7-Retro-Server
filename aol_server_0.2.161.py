...
def build_interactive_fdo():
    ...
    # 2. Button Object (Type 0x0003) - ID 0x000B (11)
    btn_label = b"Next >\x00"
    btn_obj = (
        struct.pack("<H", 0x0003) +  # Type: Button
        struct.pack("<H", 0x000B) +  # ID: 11 (referenced in Op 0x0007)
...
def send_post_auth_sequence(conn, p3):
    ...
    # 2. MOTD
    motd_text = "Welcome to AOL 7.0 Server (v0.2.161)!\n\nAuthenticating... Success!"
    motd_data = build_motd_payload(motd_text)
    send_packet(conn, p3, build_p3_packet(0x0001, 0x0002, motd_data), "MOTD Object")
    time.sleep(0.05)
    
    # ---------------------------------------------------------------------
    # 🔥 FIX: CORE SESSION COMPLETE (Op 0x000A)
    # This sets the internal Session State to "Logged In".
    # Only THEN does the client auto-close the modal password dialog.
    # ---------------------------------------------------------------------
    session_state_flags = struct.pack("<I", 0x00000003) # Flags for "Session Active/Complete"
    log(">>> 🏁 SENDING CORE_SESSION_COMPLETE (Op 0x000A)...")
    send_packet(conn, p3, build_p3_packet(0x0001, 0x000A, session_state_flags), "CORE_SESSION_COMPLETE")
    time.sleep(0.05)
    ...
    # REG_BIND_CONTEXT
    ctx_payload = struct.pack("<I", 0x00000001) 
    ...
    # REG_SET_STATE
    state_payload = struct.pack("<I", 0x00000001)
    ...
    # REG_SET_ACTION
    action_payload = struct.pack("<HHI", 0x000B, 0x0001, 0x00000001)
    log(">>> 🔗 SENDING REG_SET_ACTION (Bind Button 'Next' to Action)...")
    send_packet(conn, p3, build_p3_packet(0x001B, 0x0007, action_payload), "REG_SET_ACTION (Next Button)")
    time.sleep(0.05)
...
def start_server():
    ...
    server.listen(1)
    log(f"AOL 7 Server Alpha 0.2.161 (SESSION STATE FIX) listening on Port {PORT}")
                    ...
                    if len(data) > 12:
                        ...
                        if tool_id == 0x0001: # CORE
                            if len(payload) > 100:
                                if not p3.authenticated:
                                    ...
                                    if payload_end > payload_start:
                                        ...
                                        # -----------------------------------------------------------------
                                        # 🔥 FIX: CORE SESSION COMPLETE (Op 0x000A)
                                        # Explicitly sets Session State to "Logged In".
                                        # This closes the modal password window.
                                        # -----------------------------------------------------------------
                                        log(">>> 🏁 SENDING CORE_SESSION_COMPLETE (Op 0x000A)...")
                                        session_state_flags = struct.pack("<I", 0x00000003) 
                                        send_packet(conn, p3, build_p3_packet(0x0001, 0x000A, session_state_flags), "CORE_SESSION_COMPLETE")
                                        time.sleep(0.05)
                                        ...
                                        log(">>> 🎊 CLIENT AUTHENTICATED & MODAL WINDOW SHOULD CLOSE!")
                            ...
                            if opcode == 0x0005:
                                ...
                                submit_ok = struct.pack("<I", 0x00000001)
                                send_packet(conn, p3, build_p3_packet(0x001B, 0x0005, submit_ok), "REG_SUBMIT_RESPONSE (OK)")
